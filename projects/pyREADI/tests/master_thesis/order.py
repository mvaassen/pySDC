...
def main():
    nxs = [2 ** (i+1) - 1 for i in range(1, 12)]
    dts = [0.5 ** i for i in range(1, 4)]
    ...
    #pickle.dump([nxs, dts, sdc_err], open('/home/zam/vaassen/master_thesis/numerical_results/asymptotics_in_space/reference_sdc_newton_flameprop_ord5.p', 'wb'))
    ...
    #pickle.dump([nxs, dts, mgsdc_err], open('/home/zam/vaassen/master_thesis/numerical_results/asymptotics_in_space/sdc_fas_flameprop_ord5.p', 'wb'))

    print(dur1, dur2, dur3)

    plot_n_err(nxs, dts, mgsdc_err)
    plt.show()
...
def sdc_sim(nx, dt, expl_boundary=False):
    ...
    description = dict()
    description['problem_class'] = generalized_fisher_wtf
...
def sdc_sim_MG(nx, dt):
    ...
    problem_params['mg_restol'] = 1e-11
    problem_params['interval'] = (-10, 10)
