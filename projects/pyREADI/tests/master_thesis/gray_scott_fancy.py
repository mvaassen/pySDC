...
def main():
    nxs = [2 ** (i+1) for i in range(6, 7)]
    dts = [10 ** i for i in range(1, 2)]
...
def sdc_sim(nx, dt, expl_boundary=False):
    ...
    problem_params = dict()
    problem_params['diff_mat'] = sp.diags([[4 * 2e-5, 4 * 1e-5]], [0]).tocsr()  # diffusion coefficient
    problem_params['feedrate'] = 0.026
    problem_params['killrate'] = 0.051
    ...
    problem_params['newton_tol'] = 1e-4
    ...
    pickle.dump([xvalues, yvalues, uend.values.reshape((2, nx, nx))[0], uend.values.reshape((2, nx, nx))[1]],
                open(
                    '/home/zam/vaassen/master_thesis/numerical_results/fancy/gs_chaos.p',
                    'wb'))
...
def sdc_sim_MG(nx, dt):
    ...
    problem_params = dict()
    problem_params['diff_mat'] = sp.diags([[4 * 2e-5, 4 * 1e-5]], [0]).tocsr()  # diffusion coefficient
    problem_params['feedrate'] = 0.03
    problem_params['killrate'] = 0.062
    ...
    # pickle.dump([xvalues, yvalues, uend.values.reshape((2, nx, nx))[0], uend.values.reshape((2, nx, nx))[1]],
    #             open(
    #                 '/home/zam/vaassen/master_thesis/numerical_results/fancy/gs_lambda.p',
    #                 'wb'))
