...
def main():
    nxs = [2 ** (i+1) - 1 for i in range(1, 10)]
    ...
    aggr = [sum(diagn.diagnostics_dict['newton_wtime'][i:i+3]) for i in range(0, len(diagn.diagnostics_dict['newton_wtime']), 3)]
    print(aggr)
    plt.plot(nxs, aggr)
    plt.show()
