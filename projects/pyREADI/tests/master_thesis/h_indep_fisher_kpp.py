...
from pySDC.implementations.problem_classes.GeneralizedFisher_1D_FD_implicit_wtf_Newton_GS import generalized_fisher_wtf_newton_gs
...
def main():
    nxs = [2 ** (i+1) - 1 for i in range(3, 14)]
    dts = [0.5 ** i for i in range(1, 2)]
    ...
    # print(aggr)
    # plt.loglog(nxs, aggr,  marker='o')
    # plt.loglog(nxs, 1e-3*np.array(nxs)**2)
    # plt.show()
    #
    # plot_err(sdc_err)
    # plt.show()
    ...
    #             open(
    #                 '/home/zam/vaassen/master_thesis/numerical_results/mg_efficiency/flameprop/sdc_fas_flameprop_efficiency.p',
    #                 'wb'))
...
def sdc_sim(nx, dt, expl_boundary=False):
    ...
    problem_params = dict()
    problem_params['nu'] = 2  # diffusion coefficient
    problem_params['lambda0'] = 1
    ...
    description = dict()
    description['problem_class'] = generalized_fisher_wtf_newton_gs
...
def sdc_sim_MG(nx, dt):
    ...
    problem_params['mg_restol'] = 1e-10
    problem_params['interval'] = (-10, 10)
...
def plot_err(err_dict):
    for k, v in err_dict.iteritems():
        ...
        plt.loglog([item[0] for item in data], [1./item[0] ** 4 for item in data], linestyle='--', color='gray', label='ref')
