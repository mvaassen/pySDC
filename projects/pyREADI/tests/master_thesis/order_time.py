...
def main():
    nxs = [2 ** (i+1) - 1 for i in range(8, 10)]
...
def sdc_sim_MG(nx, dt):
    ...
    problem_params['mg_restol'] = 1e-12
    problem_params['interval'] = (-10, 10)
...
def plot_err(err_dict):
    for k, v in err_dict.iteritems():
        ...
        plt.loglog([item[0] for item in data], [item[0] ** 7 for item in data], linestyle='--', color='gray', label='ref')
