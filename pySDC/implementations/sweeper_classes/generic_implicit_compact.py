...
class generic_implicit_compact(sweeper):
    """
    ...
    def __init__(self, params):
        ...
        super(generic_implicit_compact, self).__init__(params)
    ...
    def integrate(self):
        ...
        for m in range(0, self.coll.num_nodes):
            ...
            me[-1].values += P.rdmodel.reduce(P.rdmodel.eval_m(np.array([bla[m].values])))[0]
    ...
    def compute_residual(self):
        ...
        for m in range(self.coll.num_nodes):
            ...
            l0 = P.rdmodel.extend(np.array([L.u[0].values]), L.time)
            l1 = P.rdmodel.extend(np.array([L.u[m + 1].values]), L.time + L.dt * self.coll.nodes[m])
            res[m].values += P.rdmodel.reduce(P.rdmodel.eval_m(l0 - l1))[0]
    ...
    def update_nodes(self):
        ...
        for m in range(M):
            ...
            # add initial value
            integral[m].values += P.rdmodel.extend(np.array([L.u[0].values]), L.time)[0]
            # add tau if associated
            if L.tau is not None:
                integral[m] += L.tau[m]
